$oop.postpone($widget, 'Widget', function (ns, className) {
    ...
    $widget.Widget = self
        ...
        .addMethods(/** @lends $widget.Widget# */{
            ...
            /**
             * Adds current widget to specified parent as child once.
             * - If the widget already exists on the parent (as defined by the child key) then nothing will happen
             * - Use with caution - if something else changes on the widget then this will not be reflected
             *
             * @param {$widget.Widget} parentWidget
             * @returns {$widget.Widget}
             * @see $widget.Widget#addToParent
             */
            addOnceToParent: function (parentWidget) {
                if (!parentWidget.getChild(this.childName)) {
                    this.addToParent(parentWidget);
                }
            ...
            contentMarkup: function () {
                return this.contentTemplate ?
                    ...
                    this.children.toSortedString(this.childOrder);
            },
